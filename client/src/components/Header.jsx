    ...
    <>
      ...
      <div className="flex lg:hidden items-center gap-3 w-full">
        ...
        <div className="w-full max-w-full">
          <div className={`relative p-[1px] rounded-full transition-all duration-300 ease-in-out ${
            aiSearch
            ...
            <div className="relative flex items-center gap-2 bg-zinc-900 rounded-full px-3 py-2 w-full">
              <Search className={`w-4 h-4 transition-colors duration-300 ${aiSearch ? "text-purple-400" : "text-zinc-500"}`} />
              <input
                type="text"
      ...
      <div className="hidden lg:flex justify-between items-center">
        <div className={`flex items-center gap-2 bg-zinc-800/50 rounded-lg w-96 transition-all duration-200 ${
          aiSearch
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1 max-h-64 overflow-y-auto overflow-x-hidden">
              {aiResults.map((result) => (
                <div
                  key={result.id}
                  className="p-3 bg-zinc-800/50 border border-zinc-700 rounded-lg hover:bg-zinc-700/50 transition-colors cursor-pointer min-w-0"
                  onClick={() => {
                    // Close dropdown and request in-app preview for this file
                    setShowAiResults(false);
                    setAiResults([]);
                    const eventDetail = { path: result.path, name: result.name };
                    window.dispatchEvent(new CustomEvent("nova_open_preview", { detail: eventDetail }));
                  }}
                >
