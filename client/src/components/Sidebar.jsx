...
const Sidebar = ({
  ...
  return (
    <>
      ...
      >
        <aside className="w-64 h-full bg-white/50 dark:bg-zinc-900/50 backdrop-blur-md border-r border-zinc-200 dark:border-zinc-800 flex flex-col overflow-hidden">
          {/* Header */}
          <div className="px-5 py-6 border-b flex items-center justify-between border-zinc-200 dark:border-zinc-800 flex-shrink-0">
            <div className="flex items-center text-xl font-bold text-zinc-900 dark:text-white">
              <img
                src="https://res.cloudinary.com/dw87upoot/image/upload/v1765959245/Logo_Feedback_Dec_17_2025_1_bha0nd.png"
                alt="NovaDrive logo"
                className="w-12 h-12 object-contain"
              />
              <span className="text-xl font-bold text-zinc-900 dark:text-white">NovaDrive</span>
            </div>
            <button
              className="p-2 hover:bg-zinc-100 dark:bg-zinc-800 rounded-lg transition-colors cursor-pointer"
              onClick={() => setShowSidebar(false)}
            >
      ...
      {!showSidebar && (
        <button
          className="hidden lg:flex fixed left-0 top-1/2 -translate-y-1/2 items-center justify-center w-5 h-16 bg-zinc-100/80 dark:bg-zinc-800/80 hover:bg-zinc-300 dark:hover:bg-zinc-200 dark:bg-zinc-700 border border-zinc-300 dark:border-zinc-700 rounded-r-md cursor-pointer transition-colors z-20"
      ...
      <AnimatePresence>
        {showSidebar && (
          <motion.aside
            ...
            className="fixed left-0 top-0 z-50 lg:hidden w-64 h-screen bg-white/95 dark:bg-zinc-900/95 backdrop-blur-md border-r border-zinc-200 dark:border-zinc-800 flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="px-5 py-6 border-b flex items-center justify-between border-zinc-200 dark:border-zinc-800 flex-shrink-0">
              <div className="flex items-center text-xl font-bold text-zinc-900 dark:text-white">
      ...
      className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-all cursor-pointer ${
        activeView === key
          ? accent
            ...
            : "bg-white/10 text-zinc-900 dark:text-white font-semibold border border-white/20"
          : "text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100/50 dark:bg-zinc-800/50 hover:text-zinc-900 dark:text-white"
      }`}
      onClick={() => {
        setActiveView(key);
...
function renderFooter(storageInfo, formatFileSize, storagePercentage, handleLogout) {
  return (
    <div className="px-5 py-5 border-t border-zinc-200 dark:border-zinc-800 flex-shrink-0">
      <div className="text-xs">
        <div className="flex items-center gap-2 text-zinc-600 dark:text-zinc-400 font-mono mb-3">
